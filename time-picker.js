...
import '../../datetime-input/time-input.js';
 ...
 */
const TimePickerPattern = superClass => class extends DatetimePolyfillPickerMixin(TimeInputPattern(DatetimeInputMixin(TimeElementPattern(SwitchContainerMixin(DatetimeMixin(superClass)))))) { // eslint-disable-line no-unused-vars, no-undef
  ...
  _confirmedDateTimeChanged(confirmedDate, confirmedTime) {
    if (confirmedTime === undefined) {
      ...
      return; // return, if reseting, so that the picker is not itself resetted
    }
 ...
 **/
class TimePicker extends TimePickerPattern(PolymerElement) { // eslint-disable-line no-undef
